...
static uint8_t start_weight = 0;
static uint8_t end_weight = 0;
...
static void print_help_text(void)
{
    printf("Cmd Usage:\n"
                       ...
                       "  -s <starting weight> : ramp start weight\n"
                       "  -e <ending weight> : ramp ending weight\n"
...
static void process_cmd_line(int argc, char *argv[])
{
    ...
    while((opt = getopt(argc, argv, "s:e:do:f:h")) != -1)
    {
        switch (opt)
        {
            ...
            case 's':
                start_weight = strtoul(optarg, NULL, 0);
                break;

            case 'e':
                end_weight = strtoul(optarg, NULL, 0);
                break;
    ...
    if ((start_weight < MIN_WEIGHT) ||
        (start_weight > MAX_WEIGHT))
    {
        printf("Err: invalid start weight (%d). min: %d. max: %d\n",
                start_weight,
                MIN_WEIGHT,
                MAX_WEIGHT);
    ...
    if ((end_weight < MIN_WEIGHT) ||
        (end_weight > MAX_WEIGHT))
    {
        printf("Err: invalid end weight (%d). min: %d. max: %d\n",
                end_weight,
                MIN_WEIGHT,
                MAX_WEIGHT);
...
/* emulate applying weight by writing the weight and the current timestamp to
 * out to the file */
...
int main(int argc, char * argv[])
{
    ...
    /* we will always start with the starting weight */

    apply_weight(start_weight);
